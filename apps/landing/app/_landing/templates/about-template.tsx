...
export type AboutCopy = {
  /** Page-level SEO/meta — locale copy so the about page.tsx stays
   *  hardcode-free and identical across locales. */
  meta: {
    title: string;
    description: string;
    canonical: string;
    ...
    <main className={PAGE}>
      ...
      <section data-section="about_intro">
        <div className="overflow-hidden rounded-2xl border border-border grid grid-cols-1 lg:grid-cols-[11fr_9fr]">
          ...
          <div className="order-1 lg:order-2 flex flex-col justify-center gap-6 p-5 sm:p-6 bg-[hsl(32_44%_92%)] dark:bg-[hsl(32_14%_14%)]">
            {copy.facts.map((f) => (
      ...
      <Band section="final_cta">
        <FinalCta
          texts={copy.finalCta ?? texts.finalCta}
      ...
      <LandingFooter texts={texts.footer} headerTexts={texts.header} locale={locale} routeKey="/about" />
    </main>
  );
}
